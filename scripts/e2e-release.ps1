...
function Assert-EngineVersion {
    ...
    if ($engineVersion -ne '1.2.5') {
        throw @"
Expected edge-deploy-core version 1.2.5; loaded $engineVersion.

Install the tagged release engine (not editable):
  py -m pip install "git+https://github.com/pedrochagasmaster/edge-deploy-core.git@v1.2.5"
"@
    }
}
...
function Update-ReleaseEnginePin {
    ...
    $pinNew = 'edge-deploy-core @ git+https://github.com/pedrochagasmaster/edge-deploy-core.git@v1.2.5'
...
- Bumps edge-deploy-core release extra pin from v1.1.0 to v1.2.5

## Release risk
Comment-only requirements.txt change plus release-engine pin bump to v1.2.5.
